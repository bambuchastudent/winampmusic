...
7. The receiver sees the shared tracks straight away and taps one to play it. Saving the received object, adding its playable tracks to the working library and exporting it as `.ampula` stay available behind the `⋯` menu of that view.

### Receiving a shared moment

Receiving is **listen-first**: opened a link → saw the songs → tapped a track → music plays.

- Above the list there is only the shared-music label, the moment title, the track/capture metadata and **Close**.
- The shared tracks are rendered immediately. Tapping one resolves a locally playable source and plays it in the shared-music view.
- Nothing has to be added to the working library in order to listen.
- A single `⋯` control opens the secondary actions: **Save**, **Add to library**, **.ampula file**, plus the note explaining that opening a link does not change your library.
- A track whose source cannot be resolved right now reports that on its own row, keeps its title and artist, and leaves the rest of the list — and anything already playing — untouched.
- **Save** stores the received object exactly as it arrived; a local playback match is never written into it. **Add to library** stays an explicit, separate action.
...
- listen-first receiving: shared tracks first, secondary actions behind one `⋯` control;
