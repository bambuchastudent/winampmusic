...
assert.match(pages, /--json number,title,body,url,headRefName/, 'release generation must receive the PR branch for Feature/Bugfix classification');
assert.match(pages, /node scripts\/build-release-notes\.mjs release-pr\.json release-notes\.md/, 'release notes must be rendered by the tested deterministic generator');
...
assert.match(pages, /PAGES_URL: https:\/\/bambuchastudent\.github\.io\/winampmusic\//, 'release notes generator must receive the production Pages URL');
...
assert.match(docs, /Feature.*Bugfix.*Maintenance/is, 'docs must describe compact release classification');
assert.match(docs, /one-line summary/i, 'docs must describe the short release description');
assert.match(docs, /Merged same-repository feature branches are deleted automatically/i);

console.log('release lifecycle v1.7.7: ok');
