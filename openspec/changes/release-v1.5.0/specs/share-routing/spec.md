...
## Requirement: Playlist shares return to ÁmpulaMP
Every user-facing playlist share destination SHALL be an ÁmpulaMP site URL.
...
- THEN the visible link SHALL use the ÁmpulaMP site origin
...
- THEN the fallback SHALL still use the ÁmpulaMP site origin
- AND the QR/system/copy destinations SHALL remain identical.

## Requirement: Story media is not a dead end
A v1.5.0 release/demo story asset SHALL visibly route viewers to ÁmpulaMP even if the social platform strips share metadata.
...
- THEN a readable ÁmpulaMP URL SHALL be visible in the video
- AND a scannable QR code SHALL be visible
- AND both SHALL target the canonical ÁmpulaMP site or the intended playlist share URL.

### Scenario: Platform accepts URL metadata with media
- WHEN the system share API accepts both media and URL metadata
- THEN the app SHALL pass the same ÁmpulaMP URL used by the QR code.
