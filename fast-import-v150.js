(() => {
  ...
  function parseYouTube(raw) {
    ...
    try {
      ...
      if (PLAYLIST_ID.test(playlistId)) {
        ...
        return { type: 'playlist', playlistId, videoId: /^[\w-]{11}$/.test(videoId) ? videoId : '', url: url.href };
      }
      if (host === 'youtu.be') {
      ...
      if (['shorts', 'embed', 'live'].includes(parts[0]) && /^[\w-]{11}$/.test(parts[1] || '')) return { type: 'track', videoId: parts[1], url: url.href };
    } catch {}
    return null;
  }
  ...
  function parseApple(raw) {
    try {
      ...
      if ((parts.includes('album') || parts.includes('song')) && /^\d+$/.test(trackId || '')) return { type: 'track', url: url.href };
    } catch {}
    return null;
  }
  ...
  function loadScript(src, marker) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!existing) { script.src = src; script.async = true; script.dataset.ampModule = marker; document.head.appendChild(script); }
    });
  }
  ...
  async function upgradeMetadata(videoId) {
    try {
      ...
      if (typeof window.updateTrackMetadata === 'function') {
        return window.updateTrackMetadata(videoId, { title, artist });
      }
      const library = JSON.parse(localStorage.getItem('winampmusic.library.v1') || '[]');
  ...
  async function hydratePlaylistMetadata(videoIds) {
    ...
    await Promise.all(Array.from({ length: workerCount }, async () => {
      while (cursor < videoIds.length) { const id = videoIds[cursor]; cursor += 1; await upgradeMetadata(id); }
    }));
  }
  ...
  async function resolveYouTubePlaylist(playlistId) {
    ...
    mount.hidden = true; document.body.appendChild(mount);
    let probe = null; let pollId = 0; let timeoutId = 0;
    try {
      return await new Promise((resolve, reject) => {
        let settled = false;
        const settle = (callback, value) => { if (settled) return; settled = true; clearInterval(pollId); clearTimeout(timeoutId); callback(value); };
        const inspect = () => { let ids = []; try { ids = uniqueVideoIds(probe?.getPlaylist?.()); } catch {} if (ids.length) settle(resolve, ids); };
        try {
          probe = new window.YT.Player(mount.id, {
            width: '1', height: '1',
            playerVars: { autoplay: 0, controls: 0, playsinline: 1, rel: 0, origin: location.origin, listType: 'playlist', list: playlistId },
            events: {
              onReady: (event) => { try { event.target.cuePlaylist({ listType: 'playlist', list: playlistId, index: 0, startSeconds: 0 }); } catch {} inspect(); pollId = setInterval(inspect, 180); },
              onStateChange: inspect, onError: inspect,
            },
          });
          timeoutId = setTimeout(() => settle(reject, new Error('YouTube playlist did not load')), 12000);
        } catch (error) { settle(reject, error); }
      });
    } finally { clearInterval(pollId); clearTimeout(timeoutId); try { probe?.destroy?.(); } catch {} mount.remove(); }
  }

  async function importYouTubePlaylist(youtube) {
    button.disabled = true; button.textContent = 'Importing…'; hint.textContent = 'Reading YouTube playlist…';
    try {
      ...
      input.value = ''; window.playIndex?.(firstIndex); void hydratePlaylistMetadata(ids);
    } catch (error) { console.warn('[AmpMusic] YouTube playlist import failed', error); hint.textContent = 'Could not read this YouTube playlist'; }
    finally { button.disabled = false; button.textContent = 'Add & Play'; }
  }

  async function importAppleTrack(url) {
    button.disabled = true; button.textContent = 'Matching…'; hint.textContent = 'Reading Apple Music track…';
    try {
    ...
    } catch (error) { console.warn('[AmpMusic] Apple Music import failed', error); hint.textContent = 'Apple Music track import unavailable'; }
    finally { button.disabled = false; button.textContent = 'Add & Play'; }
  }
